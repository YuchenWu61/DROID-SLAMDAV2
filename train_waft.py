...
# network
from droid_netwaft import DroidNet
...
def loadpretrained_weights(model):
    droid_ckpt_path = "./droid.pth"
    droid_state_dict = OrderedDict([(k.replace("module.", ""), v) for (k, v) in torch.load(droid_ckpt_path, map_location='cpu').items()])

    oldmodel_state_dict = model.state_dict()
    for k in oldmodel_state_dict.keys():
        if k not in list(droid_state_dict.keys()):
            print(f"Randomly initialized {k}")

    droid_state_dict["update.weight.2.weight"] = droid_state_dict["update.weight.2.weight"][:2]
    droid_state_dict["update.weight.2.bias"] = droid_state_dict["update.weight.2.bias"][:2]
    droid_state_dict["update.delta.2.weight"] = droid_state_dict["update.delta.2.weight"][:2]
    droid_state_dict["update.delta.2.bias"] = droid_state_dict["update.delta.2.bias"][:2]

    oldmodel_state_dict.update(droid_state_dict)
    model.load_state_dict(oldmodel_state_dict)
    del oldmodel_state_dict
...
def train(gpu, args):
    ...
    N = args.n_frames
    model = DroidNet()
    model = loadpretrained_weights(model)
    model.cuda()
    model.train()
    ...
    train_loader = DataLoader(db, batch_size=args.batch, sampler=train_sampler, num_workers=2, pin_memory=True, prefetch_factor=2, persistent_workers=True)
