...
class MotionFilter:
    ...
    def __context_encoder(self, image, feat_pretrained):
        """ context features """
        net, inp = self.cnet(image, feat_pretrained).split([128,128], dim=2)
        return net.tanh().squeeze(0), inp.relu().squeeze(0)

    @autocast(enabled=True)
    def __feature_encoder(self, image, feat_pretrained):
        """ features for correlation volume """
        return self.fnet(image, feat_pretrained).squeeze(0)
    ...
    def track(self, tstamp, image, depth=None, intrinsics=None):
        ...
        # extract features
        gmap = self.__feature_encoder(inputs, feat_pretrained)

        ### always add first frame to the depth video ###
        if self.video.counter.value == 0:
            print("inputs[:,[0]], feat_pretrained[:,[0]]")
            print(inputs[:,[0]].shape, feat_pretrained[:,[0]].shape)
            net, inp = self.__context_encoder(inputs[:,[0]], feat_pretrained[:,[0]])
