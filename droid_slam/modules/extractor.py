...
class BasicEncoderWAFT(nn.Module):
    def __init__(self, output_dim=128, norm_fn='batch', dropout=0.0, multidim=False):
        super(BasicEncoderWAFT, self).__init__()
        self.norm_fn = norm_fn
        self.multidim = multidim
        ...
        self.fusion_conv = nn.Conv2d(4*DIM+64, 4*DIM, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x, feat_pretrained):
        b, n, c1, h1, w1 = x.shape
        x = x.view(b*n, c1, h1, w1)
        b, n, c2, h2, w2 = feat_pretrained.shape
        feat_pretrained = feat_pretrained.view(b*n, c2, h2, w2)
        ...
        x = self.fusion_conv(torch.cat([x, feat_pretrained], dim=1))
