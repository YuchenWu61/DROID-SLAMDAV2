...
class DinoV3Feature(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            # print("dinov3 input", x.shape, x.min().item(), x.max().item(), x.mean().item())
            # waft x torch.Size([1, 3, h, w]) -2.1179039478302 2.640000104904175 -0.47344639897346497
            ...
            final = F.interpolate(outs[0], size=(h//8, w//8), mode='bilinear', align_corners=True)
            bn,c,h,w = final.shape
            final = final.view(b, n, c, h, w)
            # print("dinov3 feat output", final.shape, final.min().item(), final.max().item(), final.mean().item())
        return final
