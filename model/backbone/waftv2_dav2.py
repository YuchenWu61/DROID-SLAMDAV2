...
class DepthAnythingFeature(nn.Module):
    ...
    def forward(self, x):
        ...
        @x: (B,C,H,W)
        """
        b,n,c,hraw,wraw = x.shape
        x = x.view(b*n, c, hraw, wraw)
        # print("hraw//14*14, wraw//14*14", hraw//14*14, wraw//14*14)
        x = F.interpolate(x, size=(hraw//14*14, wraw//14*14), mode='bilinear', align_corners=True)
        ...
        final = F.interpolate(outs[0], size=(hraw//8, wraw//8), mode='bilinear', align_corners=True)
